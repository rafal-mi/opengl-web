...
const renderSceneCb = now => {
  ...
  gl.uniformMatrix4fv(gTranslationLocation, true,  new Float32Array(translation));
...
const compileShaders = () => {
  ...
  gTranslationLocation = gl.getUniformLocation(shaderProgram, "gTranslation");
  if(gTranslationLocation === -1) {
    throw new Error(`Error getting uniform location of 'gTranslation'`);
  }
